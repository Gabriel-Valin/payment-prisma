...
describe(('Delete Client By id'), () => {
    ...
    it('should not be able delete client with invalid id', async () => {
        const newUser = await createUser.perform(mockUser)
        await createClient.perform({ userId: newUser.id, name: mockClient.name, email: mockClient.email, phone: mockClient.phone })
        await expect(sut.perform('not client')).rejects.toBeInstanceOf(BaseError)
    })
})